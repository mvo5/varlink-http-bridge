...
file is not itself a secret. Enable it with `--auth=api-key`; it can
be combined with SSH key auth as `--auth=api-key,ssh`, mechanisms are
tried in the given order and the first one that accepts wins.
...
start; with a certificate from a real CA drop `--pinnedpubkey` (or
replace it with `--cacert`):
...
$ curl -s --pinnedpubkey "$PIN" -H "Authorization: Bearer $API_KEY" \
