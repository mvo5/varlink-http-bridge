...
fn spawn_httpd(node_addr: &str, socket_dir: &std::path::Path, instance: Option<&str>) -> Daemon {
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_varlink-httpd"));
    cmd.args(["--insecure", "--bind", "127.0.0.1:0", "--relay"])
...
fn caller_reaches_the_bridge_through_the_relay() {
    ...
    let _bridge = spawn_httpd(&node_addr, sockets.path(), None);
    let _update_bridge = spawn_httpd(&node_addr, sockets.path(), Some("update"));
