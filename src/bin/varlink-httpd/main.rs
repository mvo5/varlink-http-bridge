...
async fn handle_ws(mut ws: WebSocket, unix: UnixStream) {
    ...
    let mut framer = VarlinkFramer::new();
    // clear() keeps the capacity, so read_buf always has spare room
    // and Ok(0) can only mean EOF
    let mut buf: Vec<u8> = Vec::with_capacity(8192);

    // read_buf is cancel-safe, so a single select loop is enough
    loop {
        tokio::select! {
            ws_msg = ws.recv() => {
            ...
            res = unix_read.read_buf(&mut buf) => {
                match res {
                    ...
                    Ok(0) => {
                        ...
                        if let Err(e) = send_ws_frames(&mut ws, framer.finish()).await {
                            warn!("ws send error: {e}");
                        }
                        break;
                    }
                    Ok(_) => {
                        let ws_frames = framer.push_varlink_bytes(&buf);
                        buf.clear();
