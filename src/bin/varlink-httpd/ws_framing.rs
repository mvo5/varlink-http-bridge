...
impl VarlinkFramer {
    ...
    pub(crate) fn observe_client_message(&mut self, data: &[u8]) {
        if self.upgraded {
            return;
        }
...
mod tests {
    ...
    fn test_upgrade_detection() {
        ...
        f.observe_client_message(b"{\"method\":\"io.systemd.Hostname.Describe\"}\0");
        assert!(!f.upgraded);
        f.observe_client_message(b"{\"upgrade\":false}\0");
        assert!(!f.upgraded);
        f.observe_client_message(b"not json at all");
        assert!(!f.upgraded);

        f.observe_client_message(b"{\"method\":\"m\",\"upgrade\":true}\0");
        assert!(f.upgraded);
    }
    ...
    fn test_upgrade_detection_trailing_whitespace() {
        // websocat appends \n after each message
        let mut f = VarlinkFramer::new();
        f.observe_client_message(b"{\"upgrade\":true}\n");
        assert!(f.upgraded);
    }
    ...
    fn test_raw_passthrough_after_upgrade() {
        let mut f = VarlinkFramer::new();
        f.observe_client_message(b"{\"upgrade\":true}\0");
        ...
        f.observe_client_message(b"\xff\xfe raw upload data");
        assert!(f.upgraded);
    }
