...
/// One object-safe stream type for every transport combination a client
/// dials (TCP, vsock, with or without TLS), so what runs on top is
/// instantiated once.
...
/// Perform a TLS handshake on an already-accepted stream. `client_store`
/// holds the CAs a client certificate is verified against; it is passed
/// per handshake so a caller can rotate its CAs without rebuilding the
/// acceptor.
///
/// # Errors
